...
public void populateMobileList()
{
	...
	try {
		...
		ResultSet myRs = myStmt.executeQuery("select distinct mobile from ( SELECT TOP 380 mobile  FROM [IVROutboundCalls] WHERE datepart(hour,getdate()) between 9 and 21 AND Mobile LIKE '05%' AND IsNumeric(Mobile) = 1  AND GETDATE() < dateadd(hour,48,[datetimestamp]) AND ID > (SELECT MAX(ID) - 7000 FROM [OutboundIVR].[dbo].[IVROutboundCalls] ) AND [isCallAttempted] < 5 and ([CallStatus] = 0 or ([CallStatus] = 1 and [CalledOn] < dateadd(hour,-3,getdate())) OR ([CallStatus] = 7 AND [CalledOn] < dateadd(hour,-3,getdate()) AND [isCallAttempted] < 2 )) AND [CustomerSelection] IS NULL and AudioFileID in(select audiofileid from BranchAudioFiles where [isivr] = 1 AND zoneid not in (select zoneid from [CallTimeExceptions] where CONVERT(varchar(5),getdate(),108) between [ExceptionTimeFrom] and [ExceptionTimeTo]))) dfsdf" );
	...
	String usr = "Rali";	String pswrd = "Smsa1234";
		...
		for (String mob : mobileList)
		{
			...
			try {
				Connection myConn = DriverManager.getConnection(dbUrl, usr, pswrd);
				Statement myStmt = myConn.createStatement();
				int myRs = myStmt.executeUpdate("Update [IVROutboundCalls] SET [StatusString] = NULL, [RequestID] = " + ReqID + "WHERE [Mobile] = " + mob   );
...

{
	 ...
	 String poststr = "info=";
	 poststr += URLEncoder.encode("eCommerce App" + phoneno);
	...
	public static void main(String[] args) throws Exception {
		...
		//httpclient.callIvr("0551633166", "SMSA", false);
